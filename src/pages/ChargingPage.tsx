...
const ChargingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        // Default to a central location if geolocation fails (London)
        setLocation({
          latitude: 51.5074,
          longitude: -0.1278,
          loading: false,
        ...
        <div className="rounded-[calc(2rem-1px)] bg-slate-950/60 px-8 py-12 backdrop-blur-xl md:px-12">
          ...
          >
            ...
            <h1 className="mt-3 text-4xl font-semibold md:text-5xl">
              {t('chargingPageTitle') || 'Charging & Navigation'}
            </h1>
            <p className="mt-4 text-base md:text-lg text-white/70">
              {t('chargingPageSubtitle') || 'Find charging stations, plan your routes and optimize your electric trips.'}
            </p>
          ...
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">
                {t('chargingMapTitle') || 'Charging Stations Near You'}
              </h2>
              <p className="mt-1 text-slate-600 dark:text-slate-400">
                {t('chargingMapSubtitle') || 'Find EV charging stations based on your current location'}
              </p>
            </div>
              ...
              <>
                {location.error && (
                  <div className="absolute top-4 left-4 right-4 z-10 rounded-lg bg-amber-100 dark:bg-amber-900/50 px-4 py-2 text-sm text-amber-800 dark:text-amber-200">
                    <span className="font-medium">{t('locationError') || 'Location unavailable'}:</span>{' '}
                    {t('usingDefaultLocation') || 'Showing default map. Allow location access for nearby stations.'}
                  </div>
                )}
                <iframe
                  src={getMapUrl()}
